...
def run_agent(client, resume_text, job_description):
    messages = [
        {"role": "system", "content": "You are an AI agent. Use your tools to fix the resume. Keep scoring and rewriting until the score is 8 or higher. Then, output the final resume text to the user."},
        {"role": "user", "content": f"Fix my resume.\nJob Description: {job_description}\nMy Resume: {resume_text}"}
    ]
    ...
    while True:
        ...
        for tool_call in msg.tool_calls:
            ...
            if tool_call.function.name == "score_resume":
                result = score_resume(client, 
                    args["resume_text"], 
                    args["job_text"]
                )
            elif tool_call.function.name == "rewrite_section":
                result = rewrite_section(client, 
                    args["resume_text"], 
                    args["job_text"], 
